    ...
    switch (message.Message_type()) {
        case Codes::Message_type::Aerator_set_speed: {
            ...
            if (!set_speed.Interpret_data(message.data)) {
                Logger::Print("Aerator_set_speed interpretation failed");
                return false;
            }

            Logger::Print(emio::format("Aerator speed set to: {:03.1f}", set_speed.speed));
        ...
        case Codes::Message_type::Aerator_get_speed_request: {
            ...
            Logger::Print(emio::format("Aerator pump speed requested, response: {:03.1f}", speed_response.speed));
        ...
        case Codes::Message_type::Aerator_set_flowrate: {
            ...
            if (!set_flowrate.Interpret_data(message.data)) {
                Logger::Print("Aerator_set_flowrate interpretation failed");
                return false;
            }

            Logger::Print(emio::format("Aerator pump flowrate set to: {:03.1f}", set_flowrate.flowrate));
        ...
        case Codes::Message_type::Aerator_get_flowrate_request: {
            ...
            Logger::Print(emio::format("Aerator flowrate requested, response: {:03.1f}", flowrate_response.flowrate));
        ...
        case Codes::Message_type::Aerator_move: {
            ...
            if (!move_message.Interpret_data(message.data)) {
                Logger::Print("Aerator_move interpretation failed");
                return false;
            }

            Logger::Print(emio::format("Aerator pump moving, volume: {:03.1f}, flowrate: {:03.1f}", move_message.volume, move_message.flowrate));
            Move(move_message.volume, move_message.flowrate);
            return true;
        }

        case Codes::Message_type::Aerator_stop: {
            Logger::Print("Aerator pump stop requested");
            Stop();
            return true;
        }
...
float Aerator::Move(float volume_ml, float flowrate){
    ...
    Logger::Print(emio::format("Max flowrate: {:03.1f}, selected_flowrate: {:03.1f}", max_flowrate, flowrate));

    // Calculate time of pumping and set stopper executioner
    float pump_time_sec = (std::abs(volume_ml) / effective_flowrate) * 60;

    Logger::Print(emio::format("Pumping time: {:03.1f}, effective_flowrate: {:03.1f}", pump_time_sec, effective_flowrate));
