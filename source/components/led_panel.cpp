    ...
    switch (message.Message_type()){
        ...
        case Codes::Message_type::LED_get_intensity_request:{
            ...
            if (!led_get_intensity.Interpret_data(message.data)){
                Logger::Print("LED_get_intensity_request interpretation failed");
                return false;
            }
...
bool LED_panel::Get_intensity(uint8_t channel){
    ...
    Logger::Print(emio::format("Get LED intensity Channel: {:d}, Intensity: {:04.2f}", channel, intensity));
...
bool LED_panel::Get_temperature(){
    ...
    Logger::Print(emio::format("LED panel temperature: {:05.2f}˚C", temp));
