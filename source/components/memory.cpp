...
std::optional<float> EEPROM_storage::Read_Aerator_max_flowrate() {
    float flowrate{};
    auto opt = Read_record(Record_name::Aerator_max_flowrate);
    if (!opt) {
        return std::nullopt;
    }
    const auto& data = *opt;
...
float EEPROM_storage::Write_Aerator_max_flowrate(float flowrate) {
    std::vector<uint8_t> data(sizeof(float));
    std::copy(reinterpret_cast<uint8_t *>(&flowrate),
          reinterpret_cast<uint8_t *>(&flowrate) + sizeof(float),
          data.begin());
    Write_record(Record_name::Aerator_max_flowrate, data);
    return flowrate;
}
