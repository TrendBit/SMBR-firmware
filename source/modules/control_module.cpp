    ...
    board_thermistor(new Thermistor(new ADC_channel(ADC_channel::RP2040_ADC_channel::CH_1, 3.30f), 3950, 10000, 25, 5100))
{
    ...
    led_panel_cli = new LED_panel_cli(led_panel, base_module_cli);
    heater_cli = new Heater_cli(heater, base_module_cli);
}
...
void Control_module::Setup_cli(CLI_service& cli) const {
    if(led_panel_cli){
        led_panel_cli->Connect_to_cli(cli);
    }
    if(heater_cli){
        heater_cli->Connect_to_cli(cli);
    }
}