...
void Sensor_module::Setup_components(){
    ...
    fluorometer_cli = new Fluorometer_cli(fluorometer, base_module_cli);
    spectrophotometer_cli = new Spectrophotometer_cli(spectrophotometer, base_module_cli);
    bottle_temperature_cli = new Bottle_temperature_cli(bottle_temperature, base_module_cli);
}
...
void Sensor_module::Setup_cli(CLI_service& cli) const {
    if(fluorometer_cli){
        fluorometer_cli->Connect_to_cli(cli);
    }
    if(spectrophotometer_cli){
        spectrophotometer_cli->Connect_to_cli(cli);
    }
    if(bottle_temperature_cli){
        bottle_temperature_cli->Connect_to_cli(cli);
    }
}